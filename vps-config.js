...
// Cấu hình
const CONFIG = {
  vpsListFile: path.join(__dirname, "vps-config.txt"),
...
const parseVpsLine = (line) => {
  ...
  if (parts.length !== 2) {
    throw new Error(
      `Định dạng không hợp lệ: ${line}. Định dạng đúng là "ip:name"`
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    process.on("close", (code) => {
      if (code === 0) {
        ...
        log(
          `❌ Lỗi khi tạo cấu hình rclone cho ${configName}, mã thoát: ${code}`
        );
  ...
  try {
    ...
    for (const vpsLine of vpsList) {
      try {
        ...
        // Tạo cấu hình FTP
        await createRcloneConfig(name, ip, CONFIG.ftpUser, CONFIG.ftpPassword);
      } catch (error) {
