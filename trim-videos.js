...
const trimVideo = async (inputFile, outputFile, duration) => {
  return new Promise((resolve, reject) => {
    ...
    ffmpeg(inputFile)
      ...
      .on("end", () => {
        console.log(`\nĐã cắt xong: ${path.basename(inputFile)} -> ${path.basename(outputFile)}`);
        resolve();
      })
      .on("error", (err) => {
...
const trimAllVideos = async () => {
  try {
    ...
    if (files.length === 0) {
      console.log(`Không tìm thấy file video nào trong thư mục: ${inputFolder}`);
      return;
    }
...
const main = async () => {
  if (replaceOriginals) {
    console.log('⚠️ CHẾ ĐỘ THAY THẾ: Các video gốc sẽ bị thay thế bằng phiên bản đã cắt');
    ...
    console.log(`Đã sao lưu ${files.length} video gốc vào thư mục: ${backupFolder}`);
  }
